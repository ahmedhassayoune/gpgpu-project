...
void dilation_impl(uint8_t* input, uint8_t* output, int width, int height, int stride, int pixel_stride){
    // top line (1/7)
    for (int y = 0; y < height && y < 3; ++y)
    {
        uint8_t* output_lineptr = (uint8_t*) (output + y * stride);
...
void opening_impl_inplace(uint8_t* buffer, int width, int height, int stride, int pixel_stride)
    ...
    erosion_impl(buffer, other_buffer, width, height, stride, pixel_stride);

    dilation_impl(other_buffer, buffer, width, height, stride, pixel_stride);
